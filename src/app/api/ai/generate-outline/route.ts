...
const normalizeOutline = (raw: z.infer<typeof looseSchema>, allowedTypes: string[]) => {
  const normalized = {
    chapters: raw.chapters.map((rawChapter, chapterIndex) => {
      const chapterName =
        pickString(rawChapter, ['name', 'title', 'chapterName', 'chapter_title', '章节', '标题']) || `第${chapterIndex + 1}章`;
      ...
      const knowledgePoints = points
        .map((item, kpIndex) => {
          ...
          const kpName =
            pickString(kp, ['name', 'title', 'pointName', 'knowledgePointName', '知识点', '标题']) || `知识点 ${kpIndex + 1}`;
          ...
          const stem =
            pickString(qRaw, ['stem', 'question', 'prompt']) || pickString(kp, ['stem', 'question']) || `请回答：${kpName}`;

          const sentenceBase = pickString(qRaw, ['sentence']) || `请填写该知识点名称：{{blank_0}}`;
          ...
          if (questionType === 'cloze') {
            const sentence = sentenceBase.includes('{{blank_') ? sentenceBase : '请填写该知识点名称：{{blank_0}}';
...
export async function POST(req: Request) {
  try {
    ...
    const safeTypes = Array.isArray(questionTypes) && questionTypes.length
...
任务：把资料一次性拆成章节和知识点，并为每个知识点生成 1 道题。
题型范围：${safeTypes.join('、')}。
字段必须使用以下命名：
- chapters[].name
